...
│         通知時刻を設定              │
│                                   │
│    [ 09:00          ]             │ ← input[type=time]（オートフォーカス）
│                                   │
│              [ 決定 ]              │ ← 確定して閉じる
└──────────────────────────────────┘
```

- `<input type="time">` を使用。シートが開いた瞬間に `.focus()` するとモバイルでシステムキーボード（テンキー含む）が自動的に呼び出される
- 「決定」タップ → 入力値を確定してシートを閉じる
...
let notificationTime:   string             = '09:00'; // HH:mm
```
